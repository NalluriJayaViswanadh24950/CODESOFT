...
class PasswordGenerator:
    def __init__(self, root):
        ...
        self.generate_button = tk.Button(root, text="Generate Password", font=("Arial", 14), command=self.generate_password, bg="#4CAF50", fg="white")
        self.generate_button.pack(pady=10)

        self.password_label = tk.Label(root, text="Generated Password:", font=("Arial", 14), bg="#f2f2f2")
        self.password_label.pack(pady=10)

        self.password_output = tk.Entry(root, font=("Arial", 14), fg="green", width=30)
        self.password_output.pack(pady=5)

    def generate_password(self):
        try:
            length = self.length_entry.get()
            if not length.isdigit():  # Check if the input is a digit
